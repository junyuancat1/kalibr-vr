...
GridCalibrationTargetAprilgrid::GridCalibrationTargetAprilgrid(
    size_t tagRows, size_t tagCols, double tagSize, double tagSpacing, unsigned int tagStartId, unsigned int tagEndId,
...
bool GridCalibrationTargetAprilgrid::computeObservation(
  ...
  for (iter = detections.begin(); iter != detections.end();) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      // printf("removed = [%d] in min board distance\n", int(remove));
    }

    //also remove tags that are flagged as bad
    if (iter->good != 1)
      remove |= true;
    // printf("removed = [%d] in good\n", int(remove));

    //also remove if the tag ID is out-of-range for this grid (faulty detection)
    if (iter->id - _tagStartId >= (int) size() / 4){
      remove |= true;
      printf("removed = [%d] in size/4 = [%d] \n", int(remove), (int) size() / 4);
    }

    if (iter->id > _tagEndId || iter->id < _tagStartId){
      remove |= true;
      // printf("remove tag in range of[%d, %d], %d \n", _tagStartId, _tagEndId, iter->id);
    }else{
      // printf("find tag in range of[%d, %d], %d \n", _tagStartId, _tagEndId, iter->id);
    }
  ...
  if (detections.size() < _options.minTagsForValidObs) {
    success = false;
    // printf("detections size = %d, less than minTagObs %d \n", detections.size(), _options.minTagsForValidObs);
  ...
  // printf("detected apriltag id list \n");
  for (unsigned int i = 0; i < detections.size(); i++) {
    // get the tag id
    unsigned int tagId = detections[i].id - _tagStartId;
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (subpix_displacement_squarred <= _options.maxSubpixDisplacement2) {
        ...
        detected_error_flag = false;
      }
    }

    if (detected_error_flag == false){
       for (int j = 0; j < 4; j++)
        outCornerObserved[pIdx[j]] = false;
    }else{
      printf("%d [%f, %f], [%f, %f], [%f, %f], [%f, %f] \n", tagId + _tagStartId, outImagePoints.row(pIdx[0]).x(), outImagePoints.row(pIdx[0]).y(), outImagePoints.row(pIdx[1]).x(), outImagePoints.row(pIdx[1]).y(), outImagePoints.row(pIdx[2]).x(), outImagePoints.row(pIdx[2]).y(), outImagePoints.row(pIdx[3]).x(), outImagePoints.row(pIdx[3]).y());
    }
