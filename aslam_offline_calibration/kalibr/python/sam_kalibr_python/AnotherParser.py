...
class AprilBox:
    def __init__(self, boardID, markerID, top_left, top_right, bottom_left, bottom_right):
        ...
        print("y_line_pos = {} / 6 - 6 * ({} - 1) = {}".format(markerID, self.boardId, self.y_line_pos))

        self.bottom_left3d = [self.x_line_pos * 0.072 * (1 + 0.2),  self.y_line_pos * 0.072 * (1 + 0.2) , 0.]
        self.bottom_right3d = [self.bottom_left3d[0] + 0.072,  self.bottom_left3d[1] , 0.]
        self.top_left3d = [self.bottom_left3d[0],  self.bottom_left3d[1] + 0.072 , 0.]
...
if __name__ == "__main__":    
    ...
    # 底板
    target_board1_file = "april_6x6_1.yaml"
    
    cmd_parse = "python3 ./ParseApril.py --target {} --cam ~/kalibr/devel/lib/kalibr/camchain-precision_camera.yaml >> {}"
    
    cmd_parse_april_board1 = cmd_parse.format(target_board1_file, "finded_tag_indexs_board1.txt")
    
    os.system(cmd_parse_april_board1)
            

    sm.setLoggingLevel(sm.LoggingLevel.Info)
    targetConfig = kc.ConfigReader.CalibrationTargetParameters(target_board1_file)
    ...
    fname = "finded_tag_indexs_board1.txt"
    numbers = []
    ...
    for i in range(0,6):
        ...
        for j in range(0,6):
            ...
            if tag_id in numbers:
                ...
                april_box_list.append(AprilBox(1, tag_id, cornersImage[left_up_corner_index], cornersImage[right_up_corner_index], cornersImage[left_bottom_corner_index], cornersImage[right_bottom_corner_index]))
